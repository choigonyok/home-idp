...
func (c *DeployManagerKubeClient) ApplyKanikoBuildJob(img, pusher string) error {
	yml := manifest.GetKanikoJobManifest(docker.NewDockerImage(img, pusher), env.Get("HOME_IDP_GIT_REPO"))
